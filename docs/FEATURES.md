...
- [八、核心功能七:仪表盘统计](#八核心功能七仪表盘统计)
- [九、前端七大页面功能详解](#九前端七大页面功能详解)
- [十、部署与工程化](#十部署与工程化)
- [附录:技术栈速查表](#附录技术栈速查表)
...
│  仪表盘 / 公司管理 / 职位配置 / 招聘信息 / 任务历史 / 执行过程 / 设置  │
└───────────────┬───────────────────────────────────────────────┘
                │ axios → /api 代理 (Vite)
┌───────────────▼───────────────────────────────────────────────┐
...
│  └────────────┘         ▼              ┌──────────────┐        │
│  ┌────────────┐  ┌────────────┐        │ tools        │        │
│  │scheduler   │  │ models     │        │ serpapi_search│       │
...
| 邮件(待实现) | aiosmtplib + jinja2 | 通知 | requirements 已列入 |

---

## 附:已知待完善项(来自 README 路线图)

- 邮件通知:搜索完成后汇总新增数发到 `EMAIL_TO_LIST`(依赖已列入,逻辑待实现)。
